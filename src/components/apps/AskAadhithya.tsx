...
const STARTER_QUESTIONS = [
  "What projects has Aadhithya built?",
...
export default function AskAadhithya() {
  ...
  const handleSubmit = async (question: string) => {
    ...
    try {
      const response = await fetch("/api/ask-Aadhithya", {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !response.body) {
        let errorMessage =
          "Ask Aadhithya is temporarily unavailable.";
      ...
      if (accumulatedText.trim()) {
        ...
        flushStreamingMessage(
          "Ask Aadhithya could not generate a response. Please try again."
        );
      }
    } catch (error) {
      console.error("Ask Aadhithya client error:", error);

      flushStreamingMessage(
        "Ask Aadhithya is temporarily unavailable. The rest of the portfolio is still accessible."
      );
    } finally {
      if (streamUpdateRef.current !== null) {
        cancelAnimationFrame(streamUpdateRef.current);
          ...
          <div className="h-full flex flex-col items-center justify-start pt-8 max-w-2xl mx-auto space-y-6">
            <div className="text-center space-y-4">
              ...
              <h3 className="text-xl font-medium text-white">
                How can I help you learn about Aadhithya?
              </h3>
            </div>
